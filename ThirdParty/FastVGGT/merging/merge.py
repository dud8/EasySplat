...
def token_merge_bipartite2d(
    ...
    with torch.no_grad():
        ...
        if enable_protection:
            ...
            protected_mask = torch.zeros(
                N, device=metric.device, dtype=torch.bool
            )
            protected_mask[protected_indices] = True
            protected_mask_src = protected_mask[src_indices]
    ...
    def merge(
        ...
        def _scatter_reduce_mean(dst_tensor, src_tensor, index_tensor):
            n_local, dst_len_local, c_local = dst_tensor.shape
            src_len_local = src_tensor.shape[1]
            dst_out = dst_tensor.scatter_add(
                -2, index_tensor.expand(n_local, src_len_local, c_local), src_tensor
            )
            counts = torch.ones(
                (n_local, dst_len_local, 1),
                device=dst_tensor.device,
                dtype=dst_tensor.dtype,
            )
            ones = torch.ones(
                (n_local, src_len_local, 1),
                device=dst_tensor.device,
                dtype=dst_tensor.dtype,
            )
            counts = counts.scatter_add(
                -2, index_tensor.expand(n_local, src_len_local, 1), ones
            )
            return dst_out / counts
        ...
        if dst.device.type == "mps" and mode == "mean":
            dst = _scatter_reduce_mean(dst, src, dst_idx)
        else:
        ...
        if extra_tensors is not None:
            ...
            if dst_e.device.type == "mps" and mode == "mean":
                dst_e = _scatter_reduce_mean(dst_e, src_e_r, dst_idx)
            else:
        ...
        if extra_tensors_2 is not None:
            ...
            if dst_e2.device.type == "mps" and mode == "mean":
                dst_e2 = _scatter_reduce_mean(dst_e2, src_e2_r, dst_idx)
            else:
