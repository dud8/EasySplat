...
class Attention(nn.Module):
    ...
    def _chunked_attention(self, q: Tensor, k: Tensor, v: Tensor, chunk_size: int):
        B, H, N, D = q.shape
        k_t = k.transpose(-2, -1)
        outputs = []
    ...
    def forward(self, x: Tensor, pos=None, global_merging=None) -> Tensor:
        ...
        if x.device.type == "mps":
            threshold = _env_int("FASTVGGT_MPS_ATTN_THRESHOLD", 2048)
            chunk_size = _env_int("FASTVGGT_MPS_ATTN_CHUNK", 256)
            if threshold > 0 and chunk_size > 0 and N >= threshold:
                use_chunked = True

        if use_chunked:
            x = self._chunked_attention(q, k, v, chunk_size)
        else:
