...
from vggt.utils.device import empty_cache, sync_device
import time
...
class Aggregator(nn.Module):
    ...
    def forward(self, images: torch.Tensor) -> Tuple[List[torch.Tensor], int]:
        ...
        mean = self._resnet_mean.to(device=images.device, dtype=compute_dtype)
        std = self._resnet_std.to(device=images.device, dtype=compute_dtype)
        ...
        if patch_tokens.dtype != compute_dtype:
            patch_tokens = patch_tokens.to(compute_dtype)

        _, P, C = patch_tokens.shape

        # Expand camera and register tokens to match batch size and sequence length
        camera_token = slice_expand_and_flatten(
            self.camera_token.to(compute_dtype), B, S
        )
        register_token = slice_expand_and_flatten(
            self.register_token.to(compute_dtype), B, S
        )

        tokens = torch.cat([camera_token, register_token, patch_tokens], dim=1)
        if tokens.dtype != compute_dtype:
            tokens = tokens.to(compute_dtype)
        ...
        for block_num in range(self.aa_block_num):
            sync_device(tokens.device)
            empty_cache(tokens.device)
            ...
            if block_num not in block4DPT_idx:
                ...
                if concat_inter.dtype != compute_dtype:
                    concat_inter = concat_inter.to(compute_dtype)
        ...
        if "pos_original" in locals():
            del pos_original
        empty_cache(device)  # Final cleanup
