...
if [[ "$SCRIPT_SOURCE" != /dev/fd/* ]]; then
  ...
  WRAPPER="$SCRIPT_DIRECTORY/build_ceres.sh"
  IMPLEMENTATION="$SCRIPT_DIRECTORY/build_ceres_impl.sh"
  ...
  builtin exec /usr/bin/env -i PATH="$CALLER_PATH" \
    ...
    "$SCRIPT_DIRECTORY" "Ceres" "non-executable" 99 "$@" 99<&0 <<'PY'
...
if [[ "$SCRIPT_SOURCE" != "/dev/fd/$FROZEN_WRAPPER_FD" || "$SCRIPT_DIRECTORY" != /* ]]; then
  builtin printf '%s\n' "Ceres build failed: frozen wrapper identity is invalid" >&2
  exit 1
fi
...
for tool in "$BOOTSTRAP_CMAKE_BIN" "$BOOTSTRAP_NINJA_BIN" "$BOOTSTRAP_RG_BIN"; do
  if [[ "$tool" != /* || ! -x "$tool" ]]; then
    builtin printf '%s\n' \
      "Ceres build failed: cmake, ninja, and rg must resolve to external executables" >&2
    exit 1
  fi
done
